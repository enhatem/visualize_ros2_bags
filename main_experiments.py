...
topics = rosbag_extract.deserialize_rosbag('/home/elie/CORO-IMARO/M2/Semester2/Coding/my_master_thesis/visualize_ros2_bags/ros_bags/experimentation/rosbag2_2021_08_10-17_19_52/rosbag2_2021_08_10-17_19_52_0.db3')
...
for timestamp, msg in (topics["/Drone1/Status"]):
    time_ = np.append(time_, timestamp)
    starting_command = np.append(starting_command, msg.status)
    #print(msg['/Drone1/Status'])
print(starting_command)

start_time = -1
for i in range( len(starting_command) ):
    # print(starting_command[i])
    if starting_command[i] == 4 :
        start_time = time_[i]
        break
print(f'The start time of the experiment should be at {start_time}')
...
ax11.plot(wx,label='$\omega_x$')
ax11.set_ylabel('$\omega_x$ [N]')
ax11.legend()

ax12.plot(wy,label='$\omega_y$')
ax12.set_ylabel('$\omega_y$ [N]')
ax12.legend()

ax13.plot(wz,label='$\omega_z$')
